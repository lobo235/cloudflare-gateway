...
// It pings Cloudflare and returns 200 if reachable, 503 otherwise.
// This endpoint intentionally skips Bearer auth — it is called by Nomad's health check.
func (s *Server) healthHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if err := s.cf.Ping(r.Context()); err != nil {
			s.log.Error("cloudflare ping failed", "error", err, "trace_id", traceID(r.Context()))
